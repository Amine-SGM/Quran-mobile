...
pub async fn get_audio_duration(app: &AppHandle, file_path: String) -> Result<f64, String> {
    ...
    get_audio_duration_internal(app, &path).await
}

/// Estimate audio duration from file size.
/// For precise duration, ffprobe should be used. This is a fallback.
async fn get_audio_duration_internal(app: &AppHandle, file_path: &PathBuf) -> Result<f64, String> {
    let ffmpeg = crate::services::ffmpeg::FFmpegService::new();
    ffmpeg.get_duration(app, file_path).await
}
