...
Create beautiful short-form videos with Quranic recitations, background videos, and optional Arabic/English subtitles. A privacy-first, serverless application that processes everything on your device.
...
- **Export Resolution** — 720p, 1080p, 2K, 4K (desktop) / 720p, 1080p (mobile)
...
- **Desktop**: Node.js 18+, FFmpeg binaries in `desktop/bin/`
- **Mobile**: Node.js 18+, JDK 11+, Android Studio or Xcode
...
# Download FFmpeg binaries (desktop only)
...
2. **Select** — Choose a Surah and Ayah range (up to 20 on desktop, 10 on mobile)
...
See [docs/architecture.md](docs/architecture.md) for detailed system design, data flow, and platform differences.
...
| Mobile | React Native + ffmpeg-kit-https |

### Project Structure

```
quran-shorts-maker/
├── shared/                  # Shared TypeScript (no platform deps)
│   └── src/
│       ├── api/
...
- **Secure Storage** — API keys stored in platform secure storage (keychain/keystore)
...
| Scenario | Target |
|---|---|
| 1-min video, no subtitles | < 60 seconds |
| 1-min video, with subtitles | < 300 seconds |
...
- Thanks to the React Native and Electron communities
